...
class QuejaControllerTests {
	...
	void setup() {
		...
		BDDMockito.given(this.quejaService.findQuejaById(QuejaControllerTests.TEST_QUEJA_ID))
				.willReturn(this.q);
		...
		BDDMockito.given(this.residenciaService.findResidenciaByAnciano(this.anc))
				.willReturn(this.res);		
		
		BDDMockito.given(this.quejaService.findQuejasByManager(this.man))
		.willReturn(this.quejas);	
		
		quejas.forEach(x->System.out.println(x));
	...
	void testProcessCreationFormHasErrors() throws Exception {
		this.mockMvc
				.perform(MockMvcRequestBuilders.post("/quejas/new").with(csrf())
						.param("titulo", "")
						.param("descripcion", "")
						.param("anonimo", "false"))
	...
	void testShowQuejaNoAnonimo() throws Exception {
		this.mockMvc
				...
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(MockMvcResultMatchers.model().attributeExists("queja"))
				.andExpect(MockMvcResultMatchers.model().attribute("queja", Matchers.hasProperty("descripcion", Matchers.is("Prueba desc"))))
				.andExpect(MockMvcResultMatchers.model().attribute("queja", Matchers.hasProperty("titulo", Matchers.is("Prueba"))))
				.andExpect(MockMvcResultMatchers.model().attribute("queja", Matchers.hasProperty("fecha", Matchers.is(this.fecha))))
				.andExpect(MockMvcResultMatchers.model().attribute("queja", Matchers.hasProperty("anonimo", Matchers.is(false))))
				.andExpect(MockMvcResultMatchers.model().attribute("queja", Matchers.hasProperty("anciano", Matchers.hasProperty("nombre", Matchers.is("Pepe")))))
				.andExpect(MockMvcResultMatchers.model().attribute("queja", Matchers.hasProperty("anciano", Matchers.hasProperty("apellidos", Matchers.is("Sol")))))
