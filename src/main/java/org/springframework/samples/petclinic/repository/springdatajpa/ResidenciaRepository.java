...
public interface ResidenciaRepository extends CrudRepository<Residencia, String> {
	...
	@Query("SELECT r from Residencia r where r.id not in (SELECT distinct(re.id) FROM Residencia re, PeticionExcursion p, Excursion e where "
			+ "p.estado = 'aceptada' and re.id = p.residencia.id and p.excursion.id = "
			+ "e.id and e.organizador.id =:id)")
